...
DepthStencilState DisableDepthWriting
{
	...
	//Make sure others have this to all
	DepthWriteMask = ZERO;
};
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	// Calculate half size of the quad
	float halfSize = size * 0.5;

	// Calculate the coordinates of the four vertices of the quad
	...
	topLeft = origin + mul(float4(topLeft, 0), gViewInverse).xyz;
	topRight = origin + mul(float4(topRight, 0), gViewInverse).xyz;
	bottomLeft = origin + mul(float4(bottomLeft, 0), gViewInverse).xyz;
	bottomRight = origin + mul(float4(bottomRight, 0), gViewInverse).xyz;
