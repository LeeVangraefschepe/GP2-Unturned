...
void ModelAnimator::SetAnimation(const std::wstring& clipName)
{
	...
	for (auto& item : m_pMeshFilter->GetAnimationClips())
	{
	...
	Logger::LogWarning(L"No clipname found.");
	//If found,
...
void ModelAnimator::SetAnimation(UINT clipNumber)
{
	...
	{
		const auto animationClip = m_pMeshFilter->GetAnimationClips()[clipNumber];
		SetAnimation(animationClip);
	}
	else
	{
		Reset(false);
		Logger::LogWarning(L"Given clipnumber not available in AnimationClips.");
		return;
	}
	//If not,
		//	Call Reset
...
void ModelAnimator::Reset(bool pause)
{
	...
	m_Transforms = std::vector(m_pMeshFilter->GetBoneCount(), identity);
}
